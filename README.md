...
- Пользователь может сам посмотреть текущий пароль и сменить его через кнопки в `/start`.
- Бот поддерживает стикеры для ключевых событий, если задать `file_id` стикеров в `.env`.
...
`STICKER_*` - необязательные `file_id` стикеров Telegram. Чтобы получить `file_id`, отправьте нужный стикер своему боту или @RawDataBot.
...
Если пользователь поменяет пароль вручную в Nextcloud, загрузка через бота перестанет работать. Пользователь может нажать `Сменить пароль` в `/start`, либо администратор может открыть карточку пользователя и нажать `Сбросить пароль`.
