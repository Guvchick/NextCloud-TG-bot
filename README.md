...
Данные PostgreSQL хранятся в Docker volume `postgres_data`, Redis - в `redis_data`, бекапы - в `./backups`, логи - в `./logs`.
...
В админ-панели `Бекапы` можно создать сжатый PostgreSQL-бекап, создать публичный JSON-экспорт без паролей, посмотреть последние PostgreSQL-бекапы на сервере и восстановить пользователей из PostgreSQL-бекапа.
